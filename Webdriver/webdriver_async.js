...
(async function githubAutomation() {
  ...
  try {
    ...
    await driver
      .wait(
        until.elementLocated(
          By.xpath(
            "//div[@class='position-relative']/child::a[text()='Sign up']"
          )
        ),
        1000
      )
    ...
    await driver
      .findElement(
        By.className(
          "btn-mktg signup-btn  js-octocaptcha-form-submit width-full"
        )
      )
      .click();

    let registrationPage = await driver.getWindowHandle();
    ...
    await driver
      .findElement(By.name("q"))
      .sendKeys("stack overflow", Key.RETURN);
